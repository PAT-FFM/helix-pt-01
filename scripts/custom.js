...
/**
 * jQuery plugin to enable overlay
 *
 */
(function( $ ) {
  "use strict";
  var overlay = "overlay";
  var defaults = {
    overlayID: "lb__overlay",
    closeID: "lb__close_btn",
    closeLabel: "Close",
    contentID: "lb__content",
    openOverlayClass: "lb_open"
  };
  var $element,
      $overlay,
      $loadTarget,
  ...
  $[overlay] = function( element, options ) {
    $element = $(element);
    // settings from defaults or options
    settings = $.extend( defaults, options, {} );
    _private = {
      /**
       * init overlay (Singleton pattern)
       */
      init: function() {
        instance = 1;
        ...
        $overlay = $("<div>", {
          ...
          "type" :"button",
          "id": settings.closeID,
          "html": settings.closeLabel
        }).on("click", function (e) {
          e.preventDefault();
          _private.closeOverlay();
        }));
        // create target for content
        $loadTarget = $("<div>",{ "id" : settings.contentID});
        $overlay.append($loadTarget);
        $("body").append($overlay);
        overlayStatus = "created";
      },
      /**
       * toggle display of overlay by setting a class on <body>
       */
      toggleOverlay : function() {
        if (_private.isOverlayOpen()) {
          _private.closeOverlay();
        } else {
          _private.openOverlay();
        }
      },
      /**
       * helper to determine open overlay from class on body
       */
      isOverlayOpen: function() {
        return $("body").hasClass(settings.openOverlayClass);
      },
      openOverlay: function() {
        if (!_private.isOverlayOpen()) {
          $("body").addClass(settings.openOverlayClass);
          overlayStatus = "open";
        }
      },
      closeOverlay: function() {
        $("body").removeClass(settings.openOverlayClass);
        overlayStatus = "closed";
        // remove content
        $loadTarget.html("");
      },
      /**
       * load content in overlay
       * @param url
       * @param callback
       */
      loadContent : function(url, callback) {
        // activate overlay if not open
        if (!_private.isOverlayOpen()) {
          _private.toggleOverlay();
        }
        overlayStatus = "loading";
        // load content and trigger callback
     ...
     * return public API
     */
    $[overlay].open = _private.openOverlay;
    $[overlay].close = _private.closeOverlay;
    $[overlay].toggle = _private.toggleOverlay;
    $[overlay].load = _private.loadContent;
    $[overlay].getElement = _private.getElement;
    $[overlay].getStatus = _private.getStatus;
    $[overlay].getSettings = _private.getStatus;
    $[overlay].getContent = _private.getContent;
  ...
  $.fn[overlay] = function( options ) {
    $.log("called with element", options);
    return this.each(function() {
 ...
 * Image gallery
 *
 * Images are grouped in galleries by selector, default by relation attribute
 *
 * Default state of images in gallery is thumbnail view
 * Click on thumbnail load larger view on stage, URI defined in href attribute
    ...
    $(this).each(function () {
      ...
      var plugin = {
        ...
        options: $.extend({
          ...
          stageSelector: ".gallery_stage"
        }, pOptions || {}),

        initGallery: function () {
          $galleryItems = $el.find(plugin.options.galleryItemSelector);
          numItems = $galleryItems.length;
          $stage = $el.find(plugin.options.stageSelector);
          // get items
          plugin.getGalleryItems();
          // display first item on stage
          plugin.displayOnStage();
          // pager on stage
          plugin.bindPagerButtons($stage);
          // bind fullscreen button
          $stage.find(plugin.options.fullscreenBtnSelector).on("click", function(e) {
            e.preventDefault();
            plugin.loadItem();
          });
        },
        /**
         * bind pager buttons
         * every view brings their own
         */
        bindPagerButtons: function($gallery) {
          $.log("bindPagerButtons", $gallery);
          $gallery.find(plugin.options.pagerNextSelector).on("click", function(e){
            e.preventDefault();
            plugin.nextItem();
          });
          $gallery.find(plugin.options.pagerPrevSelector).on("click", function(e){
            e.preventDefault();
            plugin.prevItem();
          });
        },
        /**
         * Callback for loaded content
         */
        loadComplete: function() {
          $.log("loadComplete");
          var $gallery = $(".lb_gallery");
          /*
          // check for portait images
          var $img = $gallery.find(".gallery_image");
          var img = new Image();
          img.src = $img.attr("src");
          img.onload = function(){
            $.log("natural height:" + this.naturalHeight + ", natural width: " + this.naturalWidth);
            if (this.naturalWidth <= this.naturalHeight) {
              $img.addClass("portrait_img");
            }
          };
          */
          plugin.bindPagerButtons($gallery);
        },
        /**
         * Collect affected items in $galleryItems-Collection
         */
        getGalleryItems: function() {
          $galleryItems.each(function(index) {
            // store position in collection in data attr
         ...
         */
        nextItem: function() {
          activeIndex < numItems - 1 ? activeIndex++ : activeIndex = 0;
          plugin.newImage();
        },
        /**
         ...
         */
        prevItem: function() {
          activeIndex  > 0 ? activeIndex-- : activeIndex = numItems - 1;
          plugin.newImage();
        },
        /**
         * display new image on stage or in overlay
         */
        newImage: function() {
          // check if overlay is open
          if ($.overlay.getStatus() !== "open") {
            plugin.displayOnStage();
          } else {
            plugin.loadItem();
          }
         ...
         * load image on stage
         */
        displayOnStage: function() {
          var url = $($galleryItems[activeIndex]).attr("href");

          $.log("displayOnStage", url);
          var $stageImg = $stage.find("img");
          $stageImg.attr("src", url);
          if ($($galleryItems[activeIndex]).hasClass("portrait_img")) {
         ...
         */
        loadItem: function() {
          // get url from activeIndex data-attribute
          ...
          $loadTarget.load(url, function(response, status, xhr) {
            ...
            //
              ...
              `;
             $loadTarget.html(overlayContent);
             plugin.loadComplete();
             $(document).trigger("ajax.loaded", $loadTarget);
          }
        }
...
(function ($) {
  "use strict";
  var interactiveTable = "interactiveTable";
  ...
  // private helper functions
  $[interactiveTable] = function (element, options) {
    var $table,
      $searchfield,
      $filterWrapper,
      filter = {
    ...
    // check for filter preselection - parse from hash (example: '#2018/Sling')
    if (options.filter) {
      var filterString = decodeURIComponent(options.filter.substring(1).replace(/\+/g, '%20'));
      var filterOptions = filterString.split("/");
      var selectedYear = filterOptions[0];
      if (selectedYear && selectedYear != "") {
        filter["year"].push(selectedYear);
      }
      var selectedTag = filterOptions[1];
    ...
    _private = {
      /**
       * init table (Singleton pattern)
       */
      init: function () {
        instance = 1;
        this.request = 0;
        this.dataUrl = $element.data("jsonUrl");
        //init dataTables
        $table = $element.find("table").DataTable({
          ...
          "order": [[0, "desc"], [1, "asc"]],
          "language": {
            "emptyTable": "No matching talks found.",
            "zeroRecords": "No matching talks found."
          }
        });
        $element.find("table").hide();
        $searchfield = $element.find(".searchfield");

        $filterWrapper = $(".filter_wrapper");
        $filterWrapper.append("Loading...");
        ...
        $.fn.dataTable.ext.search.push(
          function( settings, data, dataIndex ) {
            var filterAnd = function(filter, value) {
              if (filter.length == 0) {
                return true;
              }
            ...
            var filterOr = function(filter, value) {
              if (filter.length == 0) {
                return true;
              }
              for (var i=0; i<filter.length; i++) {
            ...
            return filterOr(filter.year, data[0])
              && filterOr(filter.speaker, data[2])
              && filterOr(filter.tags, data[3]);
          }
        );
      },
      ...
      setEvents: function () {
        ...
        $searchfield.on("keyup", self.requestResults.bind(self));
      },

      requestResults: function () {
        var self = this, t = 512, searchTerm = $searchfield.val();
      ...
      getFilterOptions: function (data) {
        ...
        for (var index in filterCategories) {
          var categoryItem = filterCategories[index];
          categoryItem.items = eliminateDuplicates(categoryItem.items);
          categoryItem.items.sort();
          if (categoryItem.category == "year") {
            categoryItem.items.reverse();
          }
        }
      ...
      buildFilter: function (data) {
        var $container, $options, title, self = this;

        $filterWrapper.find("input").off();
        $filterWrapper.empty();
        for (var index in data) {
          var categoryItem = data[index];
          var categoryFilter = filter[categoryItem.category];
          // Create Categorie
          title = categoryItem.title;
          $container = $('<div class="filter_categories"></div>');
          $container.append($("<b>" + title + "</b>"));

          var expandable = categoryItem.category != "tags";

          // Create Options
          $options = $("<ul class=\"filter_options" + (expandable ? " filter_options_expandable" : "") + "\"></ul>");
          $.each(categoryItem.items, function (key, value) {
            var $li = $("<li></li>"), input;
            $li.append("<label><input/>&nbsp;</label>");
            ...
            $(input).data("category",  categoryItem.category);
            $(input).on("change", self.toggleFilter.bind(self));
            $li.find("label").append(value);
            $options.append($li);
          });
          $options.append($('<li class="toggle" data-label-expanded="less…" data-label-collapsed="more…"></li>'));
      ...
      toggleFilter: function (evt) {
        var value = evt.target.value,
          category = $(evt.target).data("category"),
        ...
        if (isChecked) {
          ...
          filterCategory.splice(filterCategory.indexOf(value), 1);
        }

        $(evt.target).closest("li").toggleClass("active", isChecked);
        $table.draw();
      ...
      applyExpandLogic: function () {

        // detect necessity for expandability
        $.each($element.find(".filter_options_expandable"), function (key, ul) {

          var $ul = $(ul),
            expandedHeight = -1,
            collapsedHeight = -1;

          ul.className = ul.className.replace(/collapsed|expanded/, '');

          $ul.addClass("expandable");

          $ul.addClass("expanded");
          expandedHeight = $ul.height();
          $ul.removeClass("expanded");

          $ul.addClass("collapsed");
          collapsedHeight = $ul.height();
          $ul.toggleClass("collapsed", collapsedHeight !== expandedHeight);

          $ul.toggleClass("expandable", collapsedHeight !== expandedHeight);
        });
        ...
        function toggle() {
          ...
          list.toggleClass("collapsed", !isCollapsed);
          list.toggleClass("expanded", isCollapsed);
        }

        $element.find(".expandable > .toggle").off().on("click", toggle);
      }
    };
...
jQuery(document).ready(function ($) {
  ...
  // simple device capabilty check for touch
